...
//Added 04-19-21 fix from wsnewman
...
//compute and return the centroid of a pointCloud
Eigen::Vector3f  PclUtils::compute_centroid(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud_ptr) {
    Eigen::Vector3f centroid;
    Eigen::Vector3f cloud_pt;   
    int npts = input_cloud_ptr->points.size();    
    centroid<<0,0,0;
...
//same thing, but arg is reference cloud instead of pointer:
Eigen::Vector3f  PclUtils::compute_centroid(pcl::PointCloud<pcl::PointXYZRGB> &input_cloud) {
    Eigen::Vector3f centroid;
    Eigen::Vector3f cloud_pt;   
    int npts = input_cloud.points.size();    
    centroid<<0,0,0;
